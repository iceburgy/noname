			...
			lib.element.card.init = function(param){
				...
				if (decadeUI.config.cardReplace){
					...
					if (!card.classList.contains('infohidden')) {
						...
						if (res.loaded !== false) {
							if (!res.loaded) {
								var url = lib.assetURL + 'extension/' + extensionName + '/image/card/' + res.name + '.png';
								res.image = new Image();
...
1.9.97.6.3：修复类似邓小艾这种判定没有标记的bug，对决模式可能正常换装备了。新增自定义势力字图，直接放到(十周年UI/image/decoration/name_你的势力名.png)，如果不存自动用字体代替。
1.9.97.6.5：修复国战模式势力名显示错误，新增新版布局。
1.9.97.9.1：新增身份面具，identity_你的身份名.png，暂时关闭pc版判定牌的信息(有bug没电脑)。
1.9.97.9.2：优化对决模式中的对抗4v4显示身份面具一样，另一个命名为identity_身份名_false.png。
