...
game.import('character',function(lib,game,ui,get,ai,_status){
	return {
		...
		translate:{
			...
			reyanzhu:'宴诛',
			reyanzhu2:'宴诛',
			reyanzhu_info:'出牌阶段限一次，你可以令一名其他角色选择一项：将装备区里的所有牌交给你并令你修改〖宴诛〗和〖兴学〗，或弃置一张牌并令下一次受到的伤害+1直到其下回合开始。（【宴诛·改】出牌阶段限一次，可以令一名其他角色下一次受到的伤害+1直到其下个回合开始。【兴学·改】结束阶段，可以令至多X名角色（X为体力上限）依次摸一张牌，然后其中手牌数大于体力值的角色依次将一张牌置于牌堆顶。）',
			...
			zishou_info:'摸牌阶段，你可以额外摸X张牌（X为场上现存势力数）。若如此做，你于本回合内使用的牌不能指定其他角色为目标。',
