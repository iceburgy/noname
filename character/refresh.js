...
game.import('character',function(lib,game,ui,get,ai,_status){
	return {
		...
		skill:{
			...
			reguanxing:{
				audio:'guanxing',
				audioname:['jiangwei','re_jiangwei','re_zhugeliang','cun_zhugeliang','gexuan'],
			...
			xunxun:{
				...
				content:function(){
					...
					var switchToAuto=function(){
						...
						if(player==game.me){
							game.lognobroadcast(player,'将',top.reverse(),'置于牌堆顶');
						}
						if(player==game.me){
							if(top.length) game.lognobroadcast(player,'将',top.reverse(),'置于牌堆顶');
							if(bottom.length) game.lognobroadcast(player,'将',bottom,'置于牌堆底');
		...
		translate:{
			...
			"new_liyu":"利驭",
			"new_liyu_info":"当你使用【杀】对一名其他角色造成伤害后，你可获得其区域里的一张牌。然后若获得的牌不是装备牌，其摸一张牌；若获得的是装备牌，则视为你对由其指定的另一名角色使用一张【决斗】。",
			"new_retuxi":"突袭",
