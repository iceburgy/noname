	...
	return {
		...
		skill:{
			...
			qiaobian1:{
				...
				content:function(){
					"step 0"
					if(!player.isOnline2()&&player!=game.me&&player.countCards('j')==0&&(!event.isMine()||!lib.config.autoskilllist.contains('qiaobian1'))){
						event.finish();
					}
					else{
			...
			guidao:{
				...
				filter:function(event,player){
					return player.countCards('hs')>0||player.countCards('e',{color:'black'})>0;
				},
				direct:true,
				content:function(){
					"step 0"
